...
class PackageMetadataTests(unittest.TestCase):
    def test_package_metadata_matches_runtime_contract(self) -> None:
        ...
        self.assertEqual("1.0.3", __version__)
    ...
    def test_sdist_includes_source_docs_tests_and_build_backend(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual("mylittleharness-1.0.3.tar.gz", sdist_name)
            with tarfile.open(Path(tmp) / sdist_name, "r:gz") as sdist:
                names = set(sdist.getnames())

        prefix = "mylittleharness-1.0.3/"
    ...
    def test_wheel_metadata_keeps_local_install_entrypoint_contract(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            wheel_name = mylittleharness_build.build_wheel(tmp)
            self.assertEqual("mylittleharness-1.0.3-py3-none-any.whl", wheel_name)
            with zipfile.ZipFile(Path(tmp) / wheel_name) as wheel:
                names = set(wheel.namelist())
                metadata = wheel.read("mylittleharness-1.0.3.dist-info/METADATA").decode("utf-8")
                entry_points = wheel.read("mylittleharness-1.0.3.dist-info/entry_points.txt").decode("utf-8")
                license_text = wheel.read("mylittleharness-1.0.3.dist-info/LICENSE").decode("utf-8")

        self.assertIn("Name: mylittleharness\n", metadata)
        self.assertIn("Version: 1.0.3\n", metadata)
        self.assertIn("License: Apache-2.0\n", metadata)
        self.assertIn("Classifier: License :: OSI Approved :: Apache Software License\n", metadata)
        self.assertIn("mylittleharness-1.0.3.dist-info/LICENSE", names)
