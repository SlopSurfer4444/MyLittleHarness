...
- Invariants: package name/version `mylittleharness`/`1.0.0`; runtime dependencies empty; stdlib build backend; console script `mylittleharness = mylittleharness.cli:main`; bootstrap inspect/package-smoke cannot publish, mutate PATH/user config, or write product-root artifacts.
