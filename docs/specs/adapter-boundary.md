...
When an active plan remains open but `phase_status` is terminal (`complete`, `blocked`, `deferred`, `abandoned`, or `skipped`) and `project/project-state.md` carries an MLH closeout/phase writeback marker, PreToolUse may also allow exact Git staging of a coherent route-produced lifecycle set. The set must contain existing lifecycle route files, include state, and include expected companions such as `project/roadmap.md`, `project/archive/plans/*.md`, `project/implementation-plan.md`, or reviewed route-created meta-feedback notes when closeout/writeback points to them; broad add, wildcards, directories, generated/runtime caches, and partial lifecycle sets remain blocked. The block message names either the exact safe staging command for that coherent set or the explicit MLH writeback dry-run route.
...
`dashboard --inspect` is a terminal-only local cockpit over repo-visible coordination inputs. It renders project-state lifecycle posture, roadmap queue/counts, work-claim records, agent-run records, handoff packets, session active-work records, worktree diagnostics, check posture, lifecycle provenance, projection cache posture, connect/readiness posture, mlhd freshness posture, and a projection summary. `dashboard --inspect --json` exposes the same posture as a structured `mylittleharness.dashboard.v1` payload for read-only tooling, including source refs, an `agentPacket` with the portable first-pass command order, a `nextLegalDryRun` route candidate, a `cachePosture` object that mirrors the adapter/check cache boundary, `acceleratorAdoption` metadata for first-contact clients, a top-level `connectReadiness` object, and a top-level `mlhd` freshness object. Dashboard detail defaults to `auto`: on small roots it may include the complete in-memory projection, while on large roots it must return a bounded degraded cockpit without rebuilding full source/link/fan-in projection data. `--detail degraded` is an explicit always-bounded view, and `--detail full` is the opt-in full projection request. The agent packet may include the same connect/readiness action packet so hook and client adapters can consume one compact first-contact shape, but nested dashboard, MCP, hook, or adapter packets must reuse bounded cache posture instead of forcing a second full projection build.

The dashboard starts no server, daemon, watcher, dispatcher, worker, hook install, network listener, or cache refresh. Optional generated projection artifacts and any future runtime cache are degraded inputs only. Default dashboard inspection must stay suitable for runtime adapters and first-contact hooks: it may use inventory, quick cache posture, dirty markers, and bounded source samples on large roots, and it may rebuild the full in-memory projection only when the selected detail mode requires it.
