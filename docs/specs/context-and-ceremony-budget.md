...
Native hook first-contact uses the same cheap-start posture. `hooks --run session-start` and `hooks --run session-start --json` may surface the dashboard agent packet, next legal dry-run candidate, MCP adoption posture, and projection/SQLite cache posture to a client at session start, including Codex-compatible `hookSpecificOutput.additionalContext`. `hooks adapter --client codex --dry-run|--apply --scope project` is the explicit project-local Codex activation rail: dry-run writes nothing, and apply may write only `.codex/hooks.json` plus `.codex/hooks/mylittleharness_session_start.py` to register a `SessionStart` command hook. `hooks --doctor` reports this native Codex adapter separately from the optional Git pre-commit shim because Git hook installation does not make a native client call session-start. That is a context shortcut, not an authority shortcut: the hook adapter must not write user-global client config, start a listener, refresh generated cache, skip canonical route reads when exact authority is needed, approve auto-continuation, or decide lifecycle, Git, dispatcher, provider, product-diff, archive, staging, commit, push, or release actions.
